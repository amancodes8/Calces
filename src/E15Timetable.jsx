...
const E15Timetable = () => {
    ...
    const fetchTimetableData = async () => {
        try {
            const savedTimetable = localStorage.getItem("e15Timetable");
            if (savedTimetable) {
                const parsedData = JSON.parse(savedTimetable);
                setTimetableData(parsedData);
                const availableDays = Object.keys(parsedData);
                setDays(availableDays);
                const today = new Date().toLocaleString("en-US", { weekday: "long" });
                setSelectedDay(today in parsedData ? today : availableDays[0]);
            } else {
    ...
    useEffect(() => {
        if (timetableData) {
            localStorage.setItem("e15Timetable", JSON.stringify(timetableData));
        }
    }, [timetableData]);
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-6 flex flex-col items-center text-gray-800 dark:bg-gray-900 dark:text-gray-100">
            <h1 className="sm:text-4xl text-xl font-bold mb-6 text-blue-600">E15 Timetable</h1>
                            ...
                            >
                                {isEditing ? (
                                    ...
                                    <>
                                        ...
                                        <p className="font-bold text-lg text-gray-700 dark:text-gray-400 mt-2">
                                            {session.subject}
                                        </p>
                                        <p className="font-extrabold text-xl text-md text-gray-500 dark:text-gray-400 mt-1">
                                            Room: {session.room}
                                        </p>
                                    </>
                                )}
